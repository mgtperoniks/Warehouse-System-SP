            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        <th wire:click="sortBy('erp_code')" class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors">
                        ...
                        <th wire:click="sortBy('name')" class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors">
                            <div class="flex items-center gap-1">
                                ITEM VARIANT
                                @if($sortField === 'name')
                                    <span class="text-[9px] ml-0.5">{{ $sortDir === 'asc' ? '▲' : '▼' }}</span>
                        ...
                        <th wire:click="sortBy('stock')" class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors">
                            <div class="flex items-center gap-1">
                                STOCK
                                @if($sortField === 'stock')
                                    <span class="text-[9px] ml-0.5">{{ $sortDir === 'asc' ? '▲' : '▼' }}</span>
                        ...
                        <th wire:click="sortBy('weekly_avg')" class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors">
                            <div class="flex items-center gap-1">
                                AVG WEEKLY
                        ...
                        <th wire:click="sortBy('days_left')" class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors">
                            <div class="flex items-center gap-1">
                                DAYS LEFT
                        ...
                        <th wire:click="sortBy('procurement_type')" class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors">
                            <div class="flex items-center gap-1">
                                PROCUREMENT TYPE
                        ...
                        <th wire:click="sortBy('inventory_class')" class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors">
                            <div class="flex items-center gap-1">
                                INVENTORY CLASS
                        ...
                        <th wire:click="sortBy('lead_time_days')" class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors">
                            <div class="flex items-center gap-1">
                                LEAD TIME
                        ...
                        <th wire:click="sortBy('status')" class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors">
                            <div class="flex items-center gap-1">
                                STATUS
                                @if($sortField === 'status')
                                    <span class="text-[9px] ml-0.5">{{ $sortDir === 'asc' ? '▲' : '▼' }}</span>
                        ...
                        <th class="px-md py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">
                            ACTIONS
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse($variants as $variant)
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                            <!-- ERP Code -->
                            <td class="px-md py-3 font-mono text-[11px] font-bold text-slate-800 select-all whitespace-nowrap">
                            ...
                            <td class="px-md py-3">
                                <div class="min-w-[200px]">
                                    ...
                                    @if($variant->brand)
                                        <p class="text-[9px] font-bold text-slate-450 uppercase mt-0.5">{{ $variant->brand }}</p>
                                    @endif
                                </div>
                            </td>

                            <!-- Stock -->
                            <td class="px-md py-3 text-xs font-bold text-slate-700 whitespace-nowrap">
                                {{ number_format($variant->total_stock) }} <span class="text-[9px] text-slate-400 font-bold uppercase">{{ $variant->unit }}</span>
                            </td>

                            <!-- Average Weekly -->
                            <td class="px-md py-3 text-xs font-bold text-slate-700">
                            ...
                            <td class="px-md py-3 text-xs font-bold text-slate-700 whitespace-nowrap">
                                @if($variant->days_left === null)
                                    <span class="text-slate-400 font-bold uppercase tracking-wider text-[9px]">No Usage</span>
                                @else
                                    {{ number_format($variant->days_left, 1) }} <span class="text-[9px] text-slate-400 font-bold uppercase">Days</span>
                                @endif
                            </td>
                            ...
                            <td class="px-md py-3">
                                <div class="w-24 relative" x-data="{ value: '{{ $variant->lead_time_days }}' }">
                                    <input 
                                        type="number" 
                                        x-model="value"
                                        ...
                                        class="w-full h-8 pr-8 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded pl-2.5 text-[11px] font-bold text-slate-700 dark:text-slate-350 focus:ring-1 focus:ring-primary/20 focus:border-primary transition-all"
                                        min="1" 
                                        max="365"
                                    >
                                    <span class="absolute right-2.5 top-1/2 -translate-y-1/2 text-[9px] font-bold text-slate-400 pointer-events-none">Days</span>
                                </div>
                            </td>

                            <!-- Status -->
                            <td class="px-md py-3 whitespace-nowrap">
                                @if($variant->health_status === 'Healthy')
                                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-250 dark:bg-emerald-950/30 dark:text-emerald-400 dark:border-emerald-900">
                                        <span class="w-1 h-1 rounded-full bg-emerald-500"></span>
                                        Healthy
                                    </span>
                                @elseif($variant->health_status === 'Warning')
                                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-amber-50 text-amber-700 border border-amber-250 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-900">
                                        <span class="w-1 h-1 rounded-full bg-amber-500"></span>
                                        Warning
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-rose-50 text-rose-700 border border-rose-250 dark:bg-rose-950/30 dark:text-rose-400 dark:border-rose-900 animate-pulse">
                            ...
                            <td class="px-md py-3 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <a href="{{ route('items.edit', $variant->id) }}" class="flex items-center justify-center w-8 h-8 rounded hover:bg-slate-100 text-slate-500 hover:text-slate-800 transition-colors" title="Edit config">
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="px-md py-12 text-center text-slate-400">
